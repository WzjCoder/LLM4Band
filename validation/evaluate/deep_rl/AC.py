...
class Actor(nn.Module):
    ...
    def forward(self,state):
       reciving_rate = np.array([state[0][0].cpu().item()], dtype=np.float32)
       delay = np.array([state[0][1].cpu().item()], dtype=np.float32)
       packet_loss = np.array([state[0][2].cpu().item()], dtype=np.float32)
       bandwidth = np.array([state[0][3].cpu().item()], dtype=np.float32)
       
       bandwidth_two = np.zeros([8], dtype=np.float32)
       delay_loss = np.zeros([2], dtype=np.float32)
       for i in range(8):
            bandwidth_two[i] = np.array([state[0][i+3].cpu().item()], dtype=np.float32)
       delay_loss[0] = delay
       delay_loss[1] = packet_loss

       reciving_rate = Variable(torch.from_numpy(reciving_rate)).to(self.device)
       delay = Variable(torch.from_numpy(delay)).to(self.device)
       packet_loss = Variable(torch.from_numpy(packet_loss)).to(self.device)
       bandwidth  = Variable(torch.from_numpy(bandwidth_two)).to(self.device)
...
class Critic(nn.Module):
    ...
    def forward(self,state):
       ...
       bandwidth_two = np.zeros([8], dtype=np.float32)
       for i in range(8):
            bandwidth_two[i] = np.array([state[0][i+3].cpu().item()], dtype=np.float32)#bandwidth[i]
