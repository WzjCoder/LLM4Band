...
class ReplayBuffer:
    ...
    def add_episode(self, episode):
        ...
        self.buffer_size += len(episode)
    
    def add_others(self, other_qos):
        self.others.extend(other_qos)

    def np_array_sample(self, state_dim, batch_size):
        transitions_idx = random.sample(range(self.buffer_size), batch_size)
        transitions = [self.buffer[idx] for idx in transitions_idx]
...
bwe_min = torch.tensor([10.])
bwe_max = torch.tensor([20000000.])